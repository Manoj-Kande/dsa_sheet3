  ...
  try {
    ...
    const [rows, streak] = await Promise.all([
      prisma.userProgress.findMany({
        ...
        select: { problemSlug: true, status: true, solvedAt: true, updatedAt: true },
      }),
    ...
    const badgeStats: BadgeStats = {
      ...
      easySolved: solved.filter(r => problemMap.get(r.problemSlug)?.difficulty === "Easy").length,
      mediumSolved: solved.filter(r => problemMap.get(r.problemSlug)?.difficulty === "Medium").length,
      hardSolved: solved.filter(r => problemMap.get(r.problemSlug)?.difficulty === "Hard").length,
      topicsCompleted,
    };
    ...
    return NextResponse.json({
      data: {
        ...
        totalRevisit: rows.filter(r => r.status === "REVISIT").length,
        currentStreak: badgeStats.currentStreak,
        longestStreak: badgeStats.longestStreak,
        heatmap, byTopic, byDifficulty,
