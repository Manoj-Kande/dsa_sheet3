...
type SRRow = { problemSlug: string; solvedAt: Date | null; updatedAt: Date; reviewCount: number | null };
...
function nextReviewDate(solvedAt: Date, count: number): Date {
  const days = INTERVALS[Math.min(count, INTERVALS.length - 1)] ?? 1;
  const d = new Date(solvedAt); d.setUTCDate(d.getUTCDate() + days); return d;
}
...
export async function GET() {
  try {
    ...
    const due = rows.map((r: SRRow) => {
      const prob = problemMap.get(r.problemSlug); if (!prob) return null;
      const reviewDate = nextReviewDate(r.solvedAt ?? r.updatedAt, r.reviewCount ?? 0);
      ...
      .sort((a, b) => b.overdueDays - a.overdueDays).slice(0, 20);

    const upcoming = rows.map((r: SRRow) => {
      const prob = problemMap.get(r.problemSlug); if (!prob) return null;
      const reviewDate = nextReviewDate(r.solvedAt ?? r.updatedAt, r.reviewCount ?? 0);
...
export async function POST(req: Request) {
  try {
    const user = await getCurrentUser();
    const { problemSlug } = await req.json() as { problemSlug: string };
    await prisma.userProgress.updateMany({ where: { userId: user.id, problemSlug }, data: { reviewCount: { increment: 1 }, solvedAt: new Date() } });
